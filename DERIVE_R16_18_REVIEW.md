# Rounds 16 / 17 / 18 — the split you ruled, and the bug you found

**For:** review of TRVM rounds 16–18 (`TRVM/governance/`, branch `merge/governance-plane`)
**Built by:** Claude (Opus 5), session 2026-08-18
**Predecessor:** your DO-NOT-COMMIT ruling on the combined Round 16
**Status:** three independent commits, each gated green on its own before the next began.

```
b475232  The apparatus is measured too                                    (round 18)
242d319  Issuance authenticated the grant, and acceptance took both …     (round 17)
583e742  The identity bound a spelling                                    (round 16)
d8cc288  The record said the footprint was the grant …                    (round 15)
```

```
cd code
node derive_battery.mjs              # 40/40 in-process
node derive_realm_battery.mjs        # 10/10 across a real worker
node probe_coresem_v03_repro.mjs     #  4/4 frozen · 5/5 live
node probe_stalegrant_v03_repro.mjs  #  5/5
node probe_issuebind_v05_repro.mjs   #  3/3 frozen · 4/4 live
node probe_derivegrant_v02_repro.mjs #  2/2 frozen · 2/2 live
```

---

## 0. The authority bug reproduced, all three consequences

Verified before touching anything, and frozen in `code/probe_issuebind_v05_repro.mjs`:

```
1 input-swap   wasIssued: true | value: 1005 | accept: {ok:true, committable:true}
2 over-inject  authority-issued value: 1005  | accept: {ok:true, committable:true}
3a no-issuer   value: 1005                   | accept: {ok:true, committable:true}
3b real reader: {ok:false, reason:"stale-read: fb granted@1 live@2"}
3b fake reader: {ok:true, committable:true}
```

Fixed exactly as ruled: `request_sem_id = H(canonical request)` recorded at issue and recomputed at
acceptance; the options bag gone with only `expected_implementation_id` requestable; the issued
request owned through `canonicalBytes` and deep-frozen; `GrantIssuer` renamed `DerivationAuthority`
because the name was describing one of its jobs; **acceptance is a method** closing over the
issuance table and the live reader, so neither proof can arrive as an argument; and `committable`
removed — it returns `{validated, fresh_at_check}`, since one call cannot establish committability
when the World can move between it returning and the caller applying.

---

## 1. Your five rulings

**C-4 → order-independent footprint.** Adopted, and you were right that my first repair was the
wrong one. `read_footprint` is a canonical dependency **set** (sorted, deduplicated); access order
with repeats lives in `read_trace`; and `read_trace` joins `implementation_id` in
`NON_SEMANTIC_RESULT_FIELDS`. The core still *fixes* evaluation order so refusals and traces
reproduce — it just no longer makes that order an identity. A result carrying a sequence where the
set is required is **refused**, not normalized on receipt, because normalizing would let two
implementations commit to different bytes and be told they agreed.

**`sub`/`mul` + signed zero.** Table-driven witnesses for all three operators — `1e308+1e308`,
`-1e308-1e308`, `1e308*2`, and a non-number operand each. Signed zero settled the way you framed it:
the canonical numeric quotient **identifies −0 with +0**, declared in `CORE_SPEC.signed_zero`, with
`mul(-1, 0)` as the witness. It was already true of the whole canonical domain and unstated, which
is exactly how C would have decided it for us. Preserving the distinction would have meant changing
`canonicalBytes`, which the World shares — a much larger blast radius for a distinction nothing
needs.

**`TRVM-TERM-CANON-v1`.** Recorded in `lowering_spike` as a separate encoding over the target term
*before heap allocation*, explicitly not `SEMSTATE-CANONICAL-v1`. That resolves the ordering
inversion I flagged, and it is the answer I could not derive from the record myself.

**No lock token.** `committable` is gone. `accept()` returns `validated` + `fresh_at_check` and the
source and the law both state that the caller must hold the World's lock across accept-and-apply.
No lock capability is exported.

**Split into 16 / 17 / 18.** Done as three commits, in your order, each with its own ledger section
and its own green gate. Round 16's manifest says in the present tense that the authority is *not*
claimed; round 17's says the World's lock composition is specified and **not built**.

---

## 2. Two findings of our own, both apparatus

**The negative battery had been running against a checker that was already red — since round 14.**
`artifacts.json` declares `tools` as well as `case_inputs`; `grid_check` requires every *declared*
artifact to exist; the case tree only ever copied `case_inputs`. So four unrelated failures preceded
every forgery. Nothing was falsely green — each case still had to find its own diagnostic — but the
baseline was not clean, and a checker that is already failing is not measuring what a negative case
believes it is. Found only by adding a case whose expected pattern didn't match and reading the rest
of the output.

That became **M-9**, the ninth species in the self-test, and it is the one a battery of forgeries
structurally cannot see: every case in such a battery is *supposed* to make the checker fail, so a
checker that was already failing looks exactly like one doing its job. Eight meta-cases require the
harness to say something; M-9 requires it to say **nothing**.

**Two of round 17's own falsifiers were wrong in the two documented ways** — one expected a phrase
from the law text rather than the diagnostic, and one wrote `authorize()` into a `grep -E` pattern,
where `()` is an empty group that can never match its own message. Both are the M-7 species, caught
by the battery refusing to call them caught.

---

## 3. Open, unchanged and unclaimed

1. **The World's lock composition is specified and not built.** `accept()` is the right shape;
   nothing yet wraps it in `lock → accept → prepared apply → receipt → unlock`.
2. **`implementation_id` is still a declared constant.** Your launcher-measurement model is in the
   roadmap; impersonation is closed, provenance is not.
3. **No derivation has been ported**, and no `prim` op exists.
4. **The lowering spike is declared, not attempted.**

## 4. What I'd ask this pass

1. **Is `fresh_at_check` the right name and the right strength?** It asserts an observation at a
   moment. The alternative is to return nothing at all beyond `ok` and force the caller to hold the
   lock to learn anything — which is stricter and possibly more honest.
2. **`read_trace` is now in `DeriveResult` but outside the semantic projection.** Should it be in
   the result at all, or should the executor emit it on a separate channel to the film plane? It is
   currently carried by a message whose whole point is that everything in it is canonical evidence.
3. **M-9 generalises uncomfortably.** "An unperturbed instrument must report nothing" applies to
   every gate in this tree, not just the negative battery. Is the bounded nine still the right
   boundary, or does that one species deserve to be applied to the kernel, the World and the bridge
   as well?
4. **The three identities in the spike.** `program_sem_id --lowering_id--> target_term_sem_id` is a
   refinement statement about *terms*. Is there a fourth for the *result* — that the derivation's
   value equals the decoded normal form — or is that the film's job rather than the lowering's?
...
```
README.md                                    this file
code/                                        8 files, run standalone on Node ≥18
gate/                                        verbatim output, incl. runs from OUTSIDE the repo
record/CORE_SPEC.json                        the frozen core as it hashes, + the semantic projection
record/grid-derivation-sections.json         9 laws + language + lowering_spike + film_planes + roadmap
...
record/negative-cases-rounds-16-18.sh        the 19 new forgeries, each with its expected diagnostic
record/rounds-16-17-18-ledger.md             §64–§84
record/commits.txt                           the three commits
MANIFEST.sha256
```

**Gate:** grid v1.19.0 — 61 entries / 342 citations · `derive_protocol.mjs` 0.5.0 · kernel PASS ·
World 0.12.0 PASS · `--check-receipt` PASS · negative battery **95/95**, against a clean baseline
for the first time since round 14 · bridge 48/48 · derive **40/40 · 10/10** · probes **2/2+2/2**,
**4/4+5/5**, **5/5**, **3/3+4/4** · harness **9/9**. `scheduler_certificate.json` byte-identical
across all three commits — `cert_id` unmoved, the calculus untouched for a thirteenth round.
...
```json
{
 ...
 "JS_IMPLEMENTATION_ID": "impl-js-derive-v0.5.0",
...
```json
{
 "_note": "TRVM/governance/invariant-grid.json v1.19.0 — derivation-relevant sections only.",
 "grid_version": "1.19.0",
 ...
 "changelog_from_1_16_0": [
  "TRVM-DERIVE-CORE-v1 FROZEN and content-bound: core_sem_id = H(canonical CORE_SPEC) is inside every program_sem_id, so an implementation assigning different meaning to add or to numeric behaviour is a different core rather than a disagreement about one program. Every v0.2.0 program id is retired while no second implementation exists.",
  "grammar validated BEFORE an id is issued — v0.2.0 gave {op:'exec', cmd:'rm -rf /'} a program_sem_id.",
  "arithmetic typed and total across add/sub/mul, each separately witnessed; -0 is identified with +0 in the canonical numeric quotient, stated because it was already true of the domain and unstated.",
  "read_footprint is a canonical DEPENDENCY SET and access order moved to read_trace, outside the semantic projection. Declaring the order semantic — the obvious first repair — would have made two correct implementations diverge over a field neither considers semantic.",
  "serialized-boundary@2 superseded by @3."
 ],
 "changelog_from_1_17_0": [
  "new law derivation.grant-issuance@1. The draft authority bound request_id -> grant_id, so swapping canonical_inputs under an untouched request_id was accepted; an options bag spread after the authority's own fields let a caller write authority content; and acceptance took the issuer and the live reader as parameters, with the issuer defaulting to absent. All three reproduced and are frozen as I-1..I-3.",
  "issuance now binds request_sem_id = H(canonical request), recomputed at acceptance. The authority is the only constructor of a request, exactly one option may be requested, and the issued request is owned and frozen.",
  "acceptance is a METHOD on the authority — the issuance table and the live World reader are closed over, so there is no argument a caller can supply to switch either off.",
  "new law derivation.footprint-freshness@1: containment is historical, freshness is temporal, both are required, freshness keys on the footprint and never on a global vclock.",
  "acceptance no longer returns `committable`, and the draft's doing so was wrong: one call cannot establish committability because the World can move between it returning and the caller applying. It returns validated + fresh_at_check, and the commit composition belongs to the World."
 ],
 "changelog_from_1_18_0": [
  "new law evidence.harness-selftest@1 and harness_selftest.sh, 9/9: the apparatus failure species that have actually occurred here now have meta-falsifiers of their own.",
  "M-9 is the species a battery of forgeries structurally cannot see — an unperturbed case tree that already fails the checker. Round 17 found it by accident after four years of cases finding their diagnostics among noise since round 14.",
  "bounded on purpose: nine enumerated shapes, no recursion into tests of tests."
 ],
 "artifact_versions": {
  "trvm_law_kernel.mjs": "1.1.0",
  "trvm_world.mjs": "0.12.0",
  "derive_protocol.mjs": "0.5.0"
 }
}
```
...
{
 "derivation_boundary": {
  "status": "v0.5.0 — protocol proven in-process and across a worker; NOT yet the production derivation path",
  "files": {
 ...
 "case_inputs": [
  ...
  "grid_check.mjs",
  "kappa_witnesses.mjs",
  "scheduler_certificate.json",
  "refinement_receipt.json",
  "golden_prehash_vectors.json",
  "trvm_world.mjs",
  "world_warrant_receipt.json",
...
## `record/negative-cases-rounds-16-18.sh`
...
PASS  implementation-id-asserted         the result carries impl-js-derive-v0.5.0, emitted by the evaluator that ran. At v0.1.0 the REQUEST carried implementation_id, nothing checked it and the result carried none (W-2)
PASS  implementation-requirement-refused a request demanding a C executor is refused BY the JS executor (implementation-mismatch: want impl-c-pretend-v9, this is impl-js-derive-v0.5.0) — the caller's field states a requirement and the executor answers it, so impersonation has no path
...
PASS  implementation-requirement-checked-on-result a result from a different executor than the one required is refused: implementation-mismatch: want impl-js-derive-v0.5.0, result claims impl-c-derive-v0.2.0
...
PASS  trace-is-outside-the-semantic-projection semantic projection = [request_id, program_sem_id, grant_id, value, witness, support, read_footprint] — the trace is kept because the film plane wants access order, and excluded because semantics do not. Same reasoning that keeps ref_interactions out of conformance identity
...
PASS  crossing-derives                 worker returned value 5 for fb=5 bias=0, resolving the program from its OWN registry by id and stamping the result impl-js-derive-v0.5.0 against grant grant-d32ef00d02de…
...
PASS  executor-asserts-implementation  a request demanding a C executor is refused by the JS worker (implementation-mismatch: want impl-c-pretend-v9, this is impl-js-derive-v0.5.0); one demanding JS runs and returns its own id. The caller states a requirement; the executor answers it
...
PASS  intent-to-acceptance             a caller's INTENT is authorized into a request by the authority, crosses to a realm holding no world, returns a claim, and is accepted (fresh_at_check true) — then the same claim is refused once fb moves 1→2 (stale-read: fb granted@1 live@2). The worker never learns the World exists
════════════════════════════════════════════════════════════════════════════════════════════════
...
BREACH    C-4 frozen-v0.2.0          read_footprint was a SEQUENCE — [["b",1],["a",1]] — so a right-to-left implementation returned [["a",1],["b",1]] for the same program: different canonical bytes, therefore foreign-result-divergence between two evaluators that computed the same value, over a field neither of them considers semantic
CONFINED  C-4 live                   the footprint is now the canonical SET [["a",1],["b",1]] and the access order survives in read_trace [["b",1],["a",1]], outside the semantic projection. The core still FIXES evaluation order so refusals and traces reproduce; it just no longer makes that order an identity
CONFINED  identity-is-semantic       psem(P) moved psem-f154679ca23… -> psem-fa4ca55b12b… because CORE_SEM_ID core-0930d6f1007… is now inside it. Every v0.2.0 program id is retired, deliberately and now, while no second implementation exists to be broken by it
====================================================================================================
...
GAP    containment-era checks         checkResult PASS · footprintWithinGrant PASS · validateForeignResult PASS — all three CORRECT, and the value 5 is now wrong in a World where fb is 9. Re-derivation against the same snapshot can never notice this
...
HOLDS  acceptance refuses             stale-read: fb granted@1 live@2 — issuance, validation and freshness in ONE call on the AUTHORITY, which closes over its own reader and issuance table so neither proof can arrive as an argument
HOLDS  unrelated-write-ignored        other@2 moved and acceptance still passes (fresh_at_check true, and NO committable field — one call cannot make a result committable, only observed fresh) — freshness keys on the FOOTPRINT, not on a global vclock. A vclock rule would invalidate every derivation on every unrelated write and quietly undo the grant/footprint separation
...
====================================================================================================
STALE-GRANT v0.3 WITNESS: 5/5 hold; the three containment-era checks pass on a stale result BY DESIGN, which is the finding
```

## `gate/probe_issuebind_v05_repro.txt`

```text
$ cd code && node probe_issuebind_v05_repro.mjs
BREACH    I-1 frozen-draft         bias 0 -> 1000 with request_id and grant_id untouched: wasIssued true, value 1005, accept ok=true committable=true — the authority accepted request content it never issued, because issuance was keyed on the GRANT
CONFINED  I-1 live                 the same swap is refused: request-not-as-issued. Issuance records request_id -> requestSemId = H(canonical request), recomputed at acceptance, so any change to any field is a different request rather than the same one with different content
BREACH    I-2 frozen-draft         authorize(intent, {canonical_inputs:{bias:1000}}) produced an AUTHORITY-ISSUED request evaluating to 1005, accepted (committable true) — `...over` was spread after every field the authority had just decided
CONFINED  I-2 live                 authorize-options-unknown: [canonical_inputs] — the options bag is gone; exactly one field may be REQUESTED by a caller, and it is a requirement on the executor rather than authority content
BREACH    I-3 frozen-draft         a wholly self-made request accepted with the issuer OMITTED (ok=true, value 1005); and after fb@1->fb@2 the real reader refuses (stale-read: fb granted@1 live@2) while a fake reader replaying the grant returns committable=true. Acceptance was a pure function whose caller supplied both proofs of authority
CONFINED  I-3 live                 self-made request -> grant-not-issued-by-this-authority; honest request -> ok with fresh_at_check true and NO committable field; after fb moves -> stale-read: fb granted@1 live@2. accept() is a method: the issuance table and the live reader are closed over, not passed in, and there is no argument a caller can supply to switch either off
CONFINED  issued-request-is-owned  mutating the issued request throws and it still reads bias 0. Defence in depth rather than the boundary: the binding is what refuses a modified request, and the freeze is what stops one being modified by accident in the authority's own process
====================================================================================================
ISSUE-BIND v0.5 REPRO: 3/3 reproduce against the frozen draft · 4/4 confined against live
```
...
```text
# make governance — verbatim, 2026-08-19T02:44Z, node v25.2.1
...
GRID-CONSISTENCY-2: PASS — registry valid (61 entries), 343 citations resolved across 16 artifacts, no banned stale claims, structure coherent with v1.19.0. [root /home/travis/ProjectAmp2/TRVM/governance]
...
NEGATIVE BATTERY: 95/95 forgeries caught
...
## `record/rounds-16-17-18-ledger.md`
