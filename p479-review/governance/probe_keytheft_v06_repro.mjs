/* ═══════════════════════════════════════════════════════════════════════════
   trvm_world.mjs — v0.6.0 — the WORLD layer: WorldRecord + Warrant v3,
...
class World {
  constructor() {
    ...
    this._lockKey = null;     // law:maintenance.capability-confinement@1
  }
  _guard(op) {
    if (this._lockKey !== null)
      throw new Error("world-write-during-maintenance: " + op + " on the authoritative world while a pass holds the lock");
  }
  lock() {
    if (this._lockKey !== null) throw new Error("world-already-locked");
    this._lockKey = H("lock|" + this.vclock + "|" + Math.random());
    return this._lockKey;
  }
  unlock(key) {
    if (key !== this._lockKey) throw new Error("world-lock-capability-refused: unlock");
    this._lockKey = null;
  }
  commit(key, fn) {   // the ONLY door through the lock — held by the transaction
    if (key !== this._lockKey) throw new Error("world-lock-capability-refused: commit");
    this._lockKey = null;
    try { return fn(); } finally { this._lockKey = key; }
  }
  put(name, value) {
    this._guard("put");
...
class Maintainer {
  ...
  pass() {
    ...
    const after = this.world.commit(lockKey, () => {
...
// ═══ AUDIT-KEY-RECOVERY: GPT's follow-up — the master key on the object ═══
const w = new World();
w.put("flag", 0);
...
m.addGround("A", { measure: "keythief", predicate: "flag", measureFn: (view) => {
  ...
  if (f === 1) {
    const stolen = realWorld._lockKey;                       // the public property
...
const rec = m.pass();
console.log("AUDIT-KEY-RECOVERY");
console.log("key readable off the object:", typeof w._lockKey !== "undefined" || "was-cleared-post-pass");
console.log("aborted:", rec.aborted, "| leak:", JSON.stringify(w.read("leak")));
console.log("real vclock:", w.vclock, "(entry", entry + ")");
console.log("CONFINED =", w.read("leak").version === 0 && w.vclock === entry + 0);
// also: internals reachable without any key at all
const w2 = new World();
w2.res.set("ghost", { bytes: "1", hash: "x", version: 99, deleted: false });
console.log("internals writable without a key:", w2.read("ghost").value === 1, "(res is a public field)");
