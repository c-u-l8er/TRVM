/* ═══════════════════════════════════════════════════════════════════════════
   ...
   into the runtime's own falsifier — and, this round, evidence artifacts
...
function buildSchedulerCertificateV2(corpusId, vectors, schedulerTable, budget, battery) {
  const cert = {
    ...
    informational: { note: "NON-AUTHORITATIVE: outside cert_id, forbidden from supporting law claims",
      generator: "trvm_law_kernel.mjs v0.6", node: process.version },
  };
...
// ═══ PROBE D: GPT's heap-id permutation witness against floatDigest ═══════
const TERM = "!&7{a,b}=X; !&9{c,d}=Y; ((a c) (b d))";
const mk = () => { const frt = new FloatRt(); const root = extrude(frt, parse(frt, TERM)); return { frt, root }; };
const swapIds = (s) => {
  const ids = [...s.frt.heap.keys()];
  const [i, j] = ids;
  const di = s.frt.heap.get(i), dj = s.frt.heap.get(j);
  s.frt.heap.clear(); s.frt.heap.set(i, dj); s.frt.heap.set(j, di);
  return s;
};
const A = mk(), B = swapIds(mk());
const dA = floatDigest(A.frt, A.root), dB = floatDigest(B.frt, B.root);
const behav = (s) => {
  const rs = findFloatRedexes(s.frt, s.root, PLANE_POOL_FREE).map(r => r.rule).sort();
  const out = normalizeFloat(s.frt, s.root, (xs) => xs[0], mulberry32(1));
  const rb = readback(s.frt, out.root);
  return { enabled: rs.join(","), termination: out.termination, steps: out.steps, interactions: s.frt.total(), nf: rb.str };
};
const bA = behav(mk()), bB = behav(swapIds(mk()));
console.log("── PROBE D: heap-id permutation vs floatDigest ──");
console.log("  digest A:", dA);
console.log("  digest B:", dB);
console.log("  digests equal:", dA === dB);
console.log("  behavior A:", JSON.stringify(bA));
console.log("  behavior B:", JSON.stringify(bB));
console.log("  behaviors equal:", JSON.stringify(bA) === JSON.stringify(bB));

// dead-heap contamination: same live state ± a dead entry must not change SEMANTIC id
const C = mk();
{
  const g = C.frt;
  const deadL = g.fresh(), deadR = g.fresh();
  g.heap.set(g.fresh(), { lab: 999, l: deadL, r: deadR, val: parse(g, "λq.q") }); // unreachable projections → dead
}
console.log("  dead-entry added: floatDigest changed:", floatDigest(C.frt, C.root) !== dA,
  "| live set unchanged:", [...liveHeap(C.frt, C.root)].length === 2);

// ═══ PROBE E: prototype semStateId — canonical live fold, discovery order ═
function liveDiscoveryOrder(frt, root) {
  // canonical order of LIVE heap dups: first-reachable in a sub-chasing
  // structural DFS from root; discovering a dup pushes its val.
  const heapByProj = new Map();
  for (const [id, d] of frt.heap) { heapByProj.set(d.l, id); heapByProj.set(d.r, id); }
...
console.log("\n── PROBE E: semStateId invariances ──");
const sA = semStateId(A.frt, A.root), sB = semStateId(B.frt, B.root), sC = semStateId(C.frt, C.root);
console.log("  id-swap:      equal:", sA === sB);
console.log("  dead-entry:   equal:", sA === sC);
const ALPHA = "!&7{p,q}=X; !&9{r,s}=Y; ((p r) (q s))";
const LABELS = "!&3{a,b}=X; !&5{c,d}=Y; ((a c) (b d))";
const mkT = (t, R = FloatRt) => { const frt = new R(); const root = extrude(frt, parse(frt, t)); return { frt, root }; };
console.log("  alpha-rename: equal:", semStateId(...Object.values(mkT(ALPHA))) === sA);
console.log("  label-biject: equal:", semStateId(...Object.values(mkT(LABELS))) === sA);
class PerturbedRt extends FloatRt { fresh() { this.fresh_ += 13; return this.fresh_; } }
console.log("  perturbed-alloc parse: equal:", semStateId(...Object.values(mkT(TERM, PerturbedRt))) === sA);

// ═══ PROBE F: lockstep dual-allocator runs — per-step semantic chain ══════
console.log("\n── PROBE F: dual-allocator lockstep, per-step semantic ids ──");
const testTerms = ["church_exp_2_2", "church_exp_3_2", "church_apply_4", "sup_app", "dup_pair", "church_not_parity_3"];
let allOk = true;
for (const name of testTerms) {
  const v = EMBEDDED_VECTORS.find(x => x.name === name);
  const P = mkT(v.term), Q = mkT(v.term, PerturbedRt);
  let steps = 0, diverged = null, execDiffered = false;
  for (;;) {
    const sp = semStateId(P.frt, P.root), sq = semStateId(Q.frt, Q.root);
    if (sp !== sq) { diverged = steps; break; }
    if (floatDigest(P.frt, P.root) !== floatDigest(Q.frt, Q.root)) execDiffered = true;
    const rp = findFloatRedexes(P.frt, P.root, PLANE_POOL_FREE);
    const rq = findFloatRedexes(Q.frt, Q.root, PLANE_POOL_FREE);
    if (rp.length === 0 && rq.length === 0) break;
    if (rp.length !== rq.length) { diverged = steps + 0.5; break; }
    const i = steps % rp.length;   // same index-based free choice on both
    const a = fireFloat(P.frt, P.root, rp[i]); P.root = a.root;
    const b = fireFloat(Q.frt, Q.root, rq[i]); Q.root = b.root;
    if (a.rule !== b.rule) { diverged = steps + 0.75; break; }
    steps++;
    if (steps > 20000) { diverged = -1; break; }
  }
  const nfP = readback(P.frt, P.root).str, nfQ = readback(Q.frt, Q.root).str;
  const ok = diverged === null && nfP === nfQ && nfP === refNorm(v.term);
  allOk &&= ok;
  console.log(`  ${name.padEnd(22)} steps=${String(steps).padEnd(3)} semantic-chain equal: ${diverged === null}  exec-ids differed en route: ${execDiffered}  NF ok: ${nfP === nfQ}`);
}
function refNorm(t) { const rt = new Rt(); return show(normalRef(rt, parse(rt, t))); }
console.log("PROBE F verdict:", allOk ? "semantic identity is allocation-portable in lockstep" : "DIVERGENCE — design needs revision");
