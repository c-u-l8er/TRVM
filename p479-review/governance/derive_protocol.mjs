...
/** A CANONICAL STRING, AND RFC 8785 SAYS A LONE SURROGATE IS AN ERROR.
 *  `JSON.stringify("\uD800")` happily returns `"\ud800"` — well-formed JSON
 *  text for a string that is not valid Unicode — so this encoder was emitting
 *  canonical bytes for a value the standard says must terminate
 *  canonicalisation. Measured against the shipped P4.1 tree:
 *  `canonicalBytes({s:"\uD800"})` → `{"s":"\ud800"}` and the same for a lone
 *  LOW surrogate. Nothing in this tree contains one, so no identity moves;
 *  what changes is that one cannot be introduced. Applies to keys as well as
 *  values, because a key is a string and an object with an invalid key is not
 *  a canonicalisable object. */
export function canonicalString(v, path = "$") {
  for (let i = 0; i < v.length; i++) {
    const c = v.charCodeAt(i);
    if (c >= 0xd800 && c <= 0xdbff) {
      const n = v.charCodeAt(i + 1);
      if (!(n >= 0xdc00 && n <= 0xdfff))
        throw new Error("not-canonical: lone high surrogate at " + path);
      i++;
    } else if (c >= 0xdc00 && c <= 0xdfff) {
      throw new Error("not-canonical: lone low surrogate at " + path);
    }
  }
...
export function canonicalBytes(v, path = "$", onPath = new Set()) {
  ...
  if (t === "string") return canonicalString(v, path);
  if (t === "object") {
    ...
    if (Array.isArray(v)) {
      ...
      out = "{" + keys.map((k) => canonicalString(k, path + ".<key>") + ":" +
