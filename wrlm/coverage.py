...
def ordering_required(witness):
    ...
    Mere overlap of touched objects is too weak a test -- almost every rewiring
    touches some node twice, so overlap would classify nearly everything as
    interacting and the tier would carry no information. The real question is
    whether an operation references an object that another operation brings into
    or takes out of existence. That is when ordering stops being a presentation
    detail and becomes part of the problem.
    """
    ops = list(witness or [])
    lifecycle = {o.get("target") if o.get("op") == "RemoveObject"
                 else (o.get("object") or {}).get("object_id")
                 for o in ops if o.get("op") in ("AddObject", "RemoveObject")}
    lifecycle.discard(None)
    for o in ops:
        if o.get("op") in ("AddEdge", "RemoveEdge"):
            e = o.get("edge") or {}
            if e.get("src") in lifecycle or e.get("dst") in lifecycle:
                return True
    return False
...
# ---------------------------------------------------------------- the ledger
OUTCOMES = ("proposed", "accepted_unique", "duplicate_case",
            "degenerate_at_base", "unsatisfiable_or_no_witness",
            "invalid_generated_world", "host_fault", "attempt_bound_exhausted")
...
class CoverageLedger(object):
    ...
    *an under-covered cell* and *a generator that cannot inhabit the cell it
    ...
    def report(self):
        filled = sum(1 for k in self.cells
                     if self.cells[k]["accepted_unique"] >= self.spec.cell_quota)
        return {"cells_touched": len(self.cells), "cells_filled": filled,
                "accepted": sum(c["accepted_unique"] for c in self.cells.values()),
                "proposed": sum(c["proposed"] for c in self.cells.values()),
