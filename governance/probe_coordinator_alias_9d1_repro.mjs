...
// ═══════════════════════════════════════════════════════════════════════════
// AUDIT-9D.1 — the coordinator was sealed; its reachable authority graph was not
//
// v0.8.0 froze the Maintainer instance and prototype and added a private
// #inPass flag refusing registration re-entry. The flag mediates METHODS. It
// does not mediate capabilities already handed out: the raw Map references
// reachable through the (frozen, but still readable) public properties, and the
// caller-owned `spec` objects that addGround stores BY REFERENCE.
//
// Three witnesses, supplied by the external audit against this exact file.
// Run: node probe_coordinator_alias_9d1_repro.mjs
// ═══════════════════════════════════════════════════════════════════════════
const results = [];
const R = (id, held, note) => { results.push({ id, held }); console.log(`${id.padEnd(30)} ${held ? "CONFINED" : "*** BREACH ***"}  ${note}`); };
console.log("AUDIT-9D.1 — reachable authority against World " + WORLD_VERSION);
console.log("=".repeat(100));

// ── 9D.1-a. the temporal guard mediates addGround(), not defs.set() ────────
{
  const w = new World(); w.put("flag", 0);
  const m = new Maintainer(w);
  m.addGround("A", { measure: "rawmap", predicate: "flag", measureFn: (view) => {
    const f = view.read("flag");
    if (f === 1) {
      m.defs.set("GHOST", { kind: "ground", cites: [],
        spec: { measure: "g", predicate: "p", measureFn: () => ({ value: 1, witness: {}, support: [] }) } });
    }
    return { value: f, witness: { f }, support: ["flag"] };
  } });
  w.put("flag", 1);
  const rec = m.pass();
  const ghostDefs = m.defs.has("GHOST");
  const ghostAfter = Object.prototype.hasOwnProperty.call(rec.after, "GHOST");
  R("9D.1-a raw-defs-map", !ghostDefs && !ghostAfter,
    `aborted:${rec.aborted} | GHOST in defs:${ghostDefs} | in state:${m.state.has("GHOST")}` +
    ` | receipt.after names GHOST:${ghostAfter}` +
    (ghostAfter ? ` (pub_version ${rec.after.GHOST.pub_version}, warrant_id ${rec.after.GHOST.warrant_id})` : ""));
}

// ── 9D.1-b. cross-node state poison: the receipt joins two authorities ─────
// The original 9D-4 poisoned the node being re-derived, which staged apply
// overwrote — the confinement was an artifact of WHICH node was chosen.
{
  const w = new World(); w.put("fa", 0); w.put("fb", 0);
  const m = new Maintainer(w);
  m.addGround("A", { measure: "A", predicate: "fa", measureFn: (view) => {
    const f = view.read("fa");
    if (f === 1) { const b = m.state.get("B"); if (b) m.state.set("B", { ...b, warrant_id: "f0rged" }); }
  ...
  const coordB = m.state.get("B");
  const receiptB = rec.after.B;
  const mixed = receiptB && receiptB.warrant_id === "f0rged" && worldB.warrant_id !== "f0rged";
  R("9D.1-b cross-node-state-poison", !mixed,
    `aborted:${rec.aborted} | coordinator B.warrant_id=${String(coordB && coordB.warrant_id).slice(0, 12)}` +
    ` | WORLD B.warrant_id=${String(worldB.warrant_id).slice(0, 12)}…` +
    ` | receipt.after.B={pub_version:${receiptB && receiptB.pub_version}, warrant_id:${String(receiptB && receiptB.warrant_id).slice(0, 12)}}` +
    ` — RECEIPT JOINS TWO DISAGREEING AUTHORITIES: ${mixed}`);
}

// ── 9D.1-c. registration aliasing: the spec is stored by reference ─────────
// No Map access, no World escape, no reflection, no prototype patch, no lock
// theft. The derivation simply retains a caller-owned object the coordinator
// continues treating as authoritative configuration.
{
  const w = new World(); w.put("fa", 0); w.put("fb", 0);
  const m = new Maintainer(w);
  const honestB = (view) => ({ value: view.read("fb"), witness: {}, support: ["fb"] });
  const maliciousB = () => ({ value: 777, witness: { pwned: true }, support: [] });
  const specB = { measure: "B", predicate: "fb", measureFn: honestB };
  const specA = { measure: "A", predicate: "fa", measureFn: (view) => {
    const f = view.read("fa");
    if (f === 1) { specB.measureFn = maliciousB; }   // retained caller alias
    return { value: f, witness: { f }, support: ["fa"] };
  } };
  m.addGround("A", specA);
  m.addGround("B", specB);
  // BOTH grounds move: B must be classified support_changed so the pass
  // RE-DERIVES it. (A first attempt moved only `fa`, leaving B `fresh` and
  // therefore replay-VALIDATED, where the swapped fn is caught by value
  // divergence — the alias was live either way, but the commit path needs
  // a genuine re-derivation to be exercised.)
  w.put("fa", 1);
  w.put("fb", 5);
  const rec = m.pass();
  const stateB = m.state.get("B");
  const worldB = w.read("warrant:B").value;
  const pwned = (stateB && stateB.value === 777) || worldB.value === 777;
  R("9D.1-c registration-aliasing", !pwned,
    `aborted:${rec.aborted} | coordinator B.value=${stateB && stateB.value}` +
    ` | WORLD B.value=${worldB.value} — an alias retained across registration replaced the` +
    ` derivation and the transaction COMMITTED it: ${pwned}`);
  // the same species one level down — and this line MUST test assignment, not
  // readability. A first draft asserted `typeof d.spec.measureFn === "function"`
  // and printed it as "directly assignable", which is true of every version and
  // therefore measured nothing. Mislabelled evidence is the defect this record
  // keeps finding in its own instruments; caught here in review.
  const assignable = (() => {
    try { m.defs.get("B").spec.measureFn = () => ({ value: 0, witness: {}, support: [] }); return true; }
    catch { return false; }
  })();
  console.log(`${"".padEnd(30)}   (m.defs.get("B").spec.measureFn assignment succeeds: ${assignable})`);
}

console.log("=".repeat(100));
const breaches = results.filter((r) => !r.held);
console.log(`AUDIT-9D.1: ${results.length - breaches.length}/${results.length} confined` +
