#!/usr/bin/env python3
# Assembles trvm_law_kernel.mjs v1.0.2 from verbatim v0.4 blocks + new sections.
# Verbatim reuse avoids transcription errors in the parser, printer, digest,
# reference driver, and embedded conformance vectors.
import re

def blk(name):  # frozen verbatim v0.4 block (see v04_verbatim/README)
    return open("v04_verbatim/" + name, encoding="utf-8").read()

HEADER = '''/* ═══════════════════════════════════════════════════════════════════════════
   trvm_law_kernel.mjs — v1.0.2 — a law-governed Interaction Calculus kernel
...
const KERNEL_VERSION = "1.0.2";
'''

# verbatim: RNG(48-56) terms(58-65) Rt(67-83) rules(85-122) whnf/normalRef(124-171)
CORE = blk("core_parse_digest.mjs")   # v0.4 lines 48-322, frozen
# verbatim: parse(172-215) show(216-234)
PARSE_SHOW = ""   # folded into core_parse_digest.mjs
# verbatim: stateDigest(236-296 incl semId) chase/isStuckApp(298-322)
DIGEST = ""       # folded into core_parse_digest.mjs
...
# verbatim AST engine: redexRule(324-351) findRedexes(353-366) applyAt(368-399)
AST_ENGINE = blk("ast_engine.mjs")
...
# verbatim: wellFormed(454-471) vectors(473-475) genTerm(476-504)
TAIL_LIB = blk("tail_lib.mjs")
...
// ── 1 · CONFORMANCE ───────────────────────────────────────────────────────
let vectorsSrc = EMBEDDED_VECTORS, vectorsFrom = "embedded";
if (process.env.TRVM_VECTORS) {
  try {
    const j = JSON.parse(readFileSync(process.env.TRVM_VECTORS, "utf8"));
    vectorsSrc = j.vectors ?? j; vectorsFrom = process.env.TRVM_VECTORS;
  } catch { /* keep embedded */ }
}
...
// residual-collapse class the same day the engine shipped).
{
  ...
  for (const term of corpus) {
    ...
    let rb; try { rb = readback(r.frt, r.root); } catch { continue; }
    tried++;
...
out = (HEADER + "\n" + CORE + PARSE_SHOW + DIGEST + PLANES + AST_ENGINE_NOTE
       + AST_ENGINE + FLOAT + "\n" + TAIL_LIB + HARNESS)
open("trvm_law_kernel_v1.mjs", "w").write(out)
print("wrote trvm_law_kernel_v1.mjs:", len(out.splitlines()), "lines")
