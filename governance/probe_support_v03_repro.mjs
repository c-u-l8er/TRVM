/* ═══════════════════════════════════════════════════════════════════════════
   trvm_world.mjs — v0.3.0 — the WORLD layer: WorldRecord + Warrant v3,
...
// ═══ AUDIT-SUPPORT-LAUNDER: GPT's witness against shipped v0.3.0 ══════════
const w = new World();
w.put("r:x", 1);
const fn = (view) => { const x = view.read("r:x"); return { value: x > 0, witness: { x }, support: ["r:x"] }; };
const honest = deriveWarrant(w, { measure: "x>0", predicate: "positivity", measureFn: fn });
const forged = JSON.parse(JSON.stringify(honest));
forged.support = [];
forged.warrant_id = warrantIdOf(forged);          // honest reseal
console.log("AUDIT-SUPPORT-LAUNDER");
const r0 = replayWarrant(w, forged, fn);
console.log("replay forged (support []):", JSON.stringify({ ok: r0.ok, reason: r0.reason }));
w.put("r:x", 2);                                   // the support resource moves
console.log("freshness honest:", freshness(w, honest).verdict, "| freshness forged:", freshness(w, forged).verdict);
const cut = freshness(w, forged).verdict === "support_intact" ? refreshWarrant(w, forged, fn) : { refreshed: false };
console.log("refreshed:", cut.refreshed, "| support_after_refresh:", JSON.stringify(cut.warrant?.support));
if (cut.refreshed) {
  const r1 = replayWarrant(w, cut.warrant, fn);
  console.log("replay after refresh:", JSON.stringify({ ok: r1.ok }));
}
// composite replay lacks even the subset check?
console.log("replayComposite has subset check:", readFileSync("trvm_world.mjs", "utf8")
  .split("function replayComposite")[1].split("function ")[0].includes("support-not-subset"));
