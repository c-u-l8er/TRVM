...
// ═══════════════════════════════════════════════════════════════════════════
// AUDIT-9D.2 — ownership that is shallow, ownership that projects, and the
// shared realm underneath both.
// Run: node probe_realm_9d2_repro.mjs
// ═══════════════════════════════════════════════════════════════════════════
const results = [];
const R = (id, held, note) => { results.push({ id, held }); console.log(`${id.padEnd(28)} ${held ? "CONFINED" : "*** BREACH ***"}  ${note}`); };
console.log("AUDIT-9D.2 — against World " + WORLD_VERSION);
console.log("=".repeat(100));

// ── 9D.2-a. ownWarrant is shallow: value/witness are shared, unfrozen ──────
{
  const w = new World(); w.put("fa", 0); w.put("fb", { x: 0 });
  const m = new Maintainer(w);
  let mutated = false;
  m.addGround("A", { measure: "A", predicate: "fa", measureFn: (view) => {
    const f = view.read("fa");
    if (f === 1) { const b = m.state.get("B");
      try { b.value.x = 999; mutated = true; } catch { mutated = false; } }
  ...
  w.put("fa", 1);
  const rec = m.pass();
  const coordB = m.state.get("B"), worldB = w.read("warrant:B").value;
  const stepB = (rec.steps || []).find((s) => s.name === "B");
  R("9D.2-a shallow-ownWarrant",
    !mutated && Object.isFrozen(coordB.value),
    `nested mutation succeeded:${mutated} | isFrozen(stored.value):${Object.isFrozen(coordB.value)}` +
    ` | coordinator B.value=${JSON.stringify(coordB.value)} vs WORLD ${JSON.stringify(worldB.value)}` +
    ` | coordinator_diverged:${!!(rec.after.B && rec.after.B.coordinator_diverged)} (ids still match, so the ID-only check cannot see it)` +
    ` | B action:${stepB && stepB.action}`);
}

// ── 9D.2-b. ownSpec is an undeclared semantic projection ──────────────────
{
  const w = new World(); w.put("fa", 5);
  const m = new Maintainer(w);
  const spec = { measure: "A", predicate: "fa", inputs: { k: 1 },
    law_refs: ["law:world.write-mediated@1"], naive: true,
    measureFn: (view) => ({ value: view.read("fa"), witness: {}, support: ["fa"] }) };
  const wr = m.addGround("A", spec);
  const stored = m.defs.get("A").spec;
  const keptAll = ["measure", "predicate", "inputs", "law_refs", "naive", "measureFn"]
    .every((k) => Object.prototype.hasOwnProperty.call(stored, k));
  R("9D.2-b ownSpec-projection", keptAll && (wr.law_refs || []).length > 0,
    `stored spec keys: [${Object.keys(stored).sort().join(",")}]` +
    ` | derived warrant law_refs: ${JSON.stringify(wr.law_refs)}` +
    ` — the alias-severing repair is also an UNDECLARED PROJECTION: inputs participates in derivation_id`);
}

// ── 9D.2-c. the shared realm: private fields are not private to primordials ─
{
  const w = new World(); w.put("flag", 1);
  let stolen = null;
  const realGet = Map.prototype.get;
  try {
    Map.prototype.get = function (k) { if (stolen === null && this instanceof Map) stolen = this; return realGet.call(this, k); };
    w.read("flag");                       // an ordinary, permitted read
  } finally { Map.prototype.get = realGet; }
  let ghosted = false, logged = null, vclockBefore = w.vclock;
  if (stolen) {
    try { stolen.set("ghost", { bytes: JSON.stringify("escaped"), hash: "x", version: 777, deleted: false });
      ghosted = w.read("ghost").value === "escaped"; } catch { ghosted = false; }
    logged = (w.log || []).some((e) => String(JSON.stringify(e)).includes("ghost"));
  }
  R("9D.2-c primordial-theft", stolen === null || !ghosted,
    `#res captured via a patched Map.prototype.get: ${stolen !== null}` +
    ` | ghost resource created: ${ghosted} (version 777 vs world vclock ${vclockBefore})` +
    ` | append-only log records it: ${logged}` +
    ` — no lock key, no reflection, no public field, no Map handed to the derivation`);
}

console.log("=".repeat(100));
const breaches = results.filter((r) => !r.held);
console.log(`AUDIT-9D.2: ${results.length - breaches.length}/${results.length} confined` +
