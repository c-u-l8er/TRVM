...
an id the oracle already shipped; the matching artifact row is named.

**As of pack v2 these are no longer the only byte-level anchors.**
`golden_prehash_vectors.json` ships the canonical signature *strings* for every
corpus state — initial and normal form, all 24 vectors — so milestone #5's
conformance target is now byte equality, not merely digest equality. Where your
digest differs, diff the signature and the divergence is visible directly. The
worked examples below remain the hand-derivable entry point; the vectors are
the exhaustive one.
...
**Shipped since pack v2**, in `golden_prehash_vectors.json.compaction`: the last
uncompacted state (73 chars) and the first compacted one, with the compacted
row's **pre-compaction string** (84 chars) given explicitly — `'#' + sha256(it)`
equals the compacted signature the oracle emitted, so you can check your
boundary against the exact string you should have hashed. Note also that a real
corpus state signs to exactly 80 characters and passes through uncompacted: the
threshold is `> 80`, not `>= 80`, and the corpus exercises it.
...
- `golden_prehash_vectors.json` (24 terms, `vectors_id` committed) — **the
  byte-level targets**: per term, the canonical signature *string* and its
  `sem_state_id` for the initial and normal-form states, plus the §5 compaction
  boundary with its pre-compaction reconstruction. Every signature sha256s to
  its own id and every `nf_id` is anchored to the refinement receipt above, so
  the vectors provably describe *this* corpus. Emitted by `extract_prehash.mjs`
  from kernel v1.1.0's `stateSignature`, which `stateDigest` is defined in terms
  of — one code path, so a vector cannot drift from the digest it explains.

**Conformance ladder (byte-level as of pack v2):**
...
3. **Canonical signature bytes** — for every corpus state, initial and normal
   form, your signature string must equal the shipped one **character for
   character** (48 targets), and sha256 of it must equal the shipped
   `sem_state_id`. This rung was digest-only in pack v1; it is the rung where a
   subtly wrong canonical form is now *localized* rather than merely detected.
