...
// ═══════════════════════════════════════════════════════════════════════════
// AUDIT-CLOSURE-ENV — the authority is inside the closure environment.
//
// This witness is FROZEN AND NOT REPAIRED, by decision. There is no mutable
// object to own: the authority is a lexical cell shared by two closures.
// Object.freeze, deepFreeze, structuredClone, canonicalBytes, #private fields,
// GuardedStore and the pass-entry snapshot are all inapplicable — JavaScript
// gives the coordinator no mechanism to enumerate, copy, canonicalize or freeze
// a function's captured environment. Freezing measureFn itself does not help.
//
// Run: node probe_closureenv_repro.mjs
// ═══════════════════════════════════════════════════════════════════════════
console.log("AUDIT-CLOSURE-ENV — against World " + WORLD_VERSION);
console.log("=".repeat(100));
{
  const w = new World(); w.put("fa", 0); w.put("fb", 0);
  const m = new Maintainer(w);
  let bias = 0;                                   // <- the entire attack surface
  const fnB = (view) => ({ value: view.read("fb") + bias, witness: { bias }, support: ["fb"] });
  m.addGround("A", { measure: "A", predicate: "fa", measureFn: (view) => {
    const f = view.read("fa");
    if (f === 1) bias = 1000;                     // A mutates the shared cell
    return { value: f, witness: { f }, support: ["fa"] };
  } });
  m.addGround("B", { measure: "B", predicate: "fb", measureFn: fnB });
  const idBefore = m.state.get("B").derivation_id;
  const valBefore = m.state.get("B").value;
  w.put("fa", 1); w.put("fb", 5);
  const rec = m.pass();
  const stepB = (rec.steps || []).find((s) => s.name === "B");
  const after = m.state.get("B");
  console.log(`aborted:                 ${rec.aborted}`);
  console.log(`B action:                ${stepB && stepB.action}`);
  console.log(`B value before/after:    ${valBefore} -> ${after.value}   (fb moved 0->5, so an honest B is 5)`);
  console.log(`B World value:           ${w.read("warrant:B").value.value}`);
  console.log(`bias:                    ${bias}`);
  console.log(`derivation_id unchanged: ${idBefore === after.derivation_id}`);
  console.log("");
  console.log("Two findings, not one:");
  console.log("  1. the coordinator cannot own a lexical environment — nothing here is an object");
  console.log("  2. derivation_id does NOT identify a derivation's semantics: same id, same");
  console.log("     registered function reference, behaviour changed from fb to fb+1000");
  console.log("=".repeat(100));
  const breached = after.value !== 5;
  console.log(`AUDIT-CLOSURE-ENV: ${breached ? "*** BREACH *** — DECLARED, NOT REPAIRED" : "confined"}`);
  console.log("An arbitrary closure is an unbounded capability container. This is an");
  console.log("architectural boundary, not a missing Object.freeze — see");
  console.log("law:derivation.environment-confinement@1, FALSIFIED by design.");
  process.exit(0);   // exits 0: this witness DOCUMENTS a limit, it does not gate the build
}
