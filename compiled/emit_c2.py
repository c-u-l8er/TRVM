...
def emit_step_c2(view):
    ...
    for line in LAW.render("c2", "mac", min_w=min([view.spinners[s][0] for s in view.spinners] or [0])):
        emit(line)
    ...
    L += LAW.render("c2", "nxt-words", nwords=nwords, cur0=cur0, nxt0=nxt0)
    fire_u = {}
    for r in ppl:
        sp, o = spec[r], idx[("counter", r)]
        if sp[0] == "onehot":
            _, p, ph = sp
            L += LAW.render("c2", "clock-onehot", o=o, p=p, ph=ph, r=r)
        elif sp[0] == "binp":
            _, p, ph, w_ = sp
            L += LAW.render("c2", "clock-binary", o=o, p=p, ph=ph, w=w_, r=r)
        else:
            _, e, w_ = sp
            L += LAW.render("c2", "clock-once", o=o, e=e, w=w_, r=r)
        if view.out_wires(r):
            fire_u[r] = "fire_%d" % o
    # ---- signal words
    edges, fires, src_of = [], [], {}
    ...
    for k in range(nwords):
        ...
        L += LAW.render("c2", "signal-word", k=k, cur0=cur0, nxt0=nxt0, expr=" | ".join(parts) if parts else "0")
    # ---- orbs
    for i_, ob in enumerate(orbs):
        po, fo = idx[("pose", ob)], idx[("fault", ob)]
        reset_slot = 5 * len(ctrl) + i_
        emit("  { /* orb %s */" % ob)
        L += LAW.render("c2", "commit-fault-reset", reset_slot=reset_slot, fo=fo)
        s = view.controller_of(ob)
        if not s:
            L += LAW.render("c2", "orb-uncontrolled", po=po, fo=fo)
            emit("  }")
            continue
        w_, n_, _ = view.spinners[s]
        ro, ci = idx[("rotor", s)], 5 * ctrl.index(s)
        L += LAW.render("c2", "commit-rotor", ro=ro, po=po, ci=ci)
        ins = view.in_wires(s)
        sel = "(int)((n%d >> %d) & 1)" % (pos[ins[0]] // WORD, pos[ins[0]] % WORD) if ins else "0"
        L += LAW.render("c2", "react", po=po, fo=fo, w=w_, n=n_, s=s, sel=sel)
        emit("  }")
    emit("}")
    return "\n".join(L) + "\n", pw
