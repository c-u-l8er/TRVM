...
def main():
    ...
    for e in range(40):
        ...
        args = (args[0], [("p", 33, 0, 0)], *args[2:])   # truth's labels
    ...
    # --- E) cost notes by representation
    print("  cost by counter representation (interactions/epoch, lazy):")
    for name, spec in [("one-hot p=3", ("periodic", 3, 0)),
                       ("binary p=33 (w=6)", ("periodic", 33, 0)),
                       ("binary p=257 (w=9)", ("periodic", 257, 100)),
                       ("once e=40 (w=6+latch)", ("once", 40))]:
        fx = FX.Fixture({"p": spec}, [], ["d"], [("p", "d")])
        t0, proj0, mf, _, _ = B2.model_films(fx, 5)
        _, costs, _ = B2.ic_films(fx, t0, proj0, 5)
        print(f"    {name:>22}: {costs[0]}")
