...
def model_projection(w, roles, wid_role, t):
    ...
    pulsers = [("p3", ob[roles["p3"]]["period"], ob[roles["p3"]]["phase"], 0),
               ("seed", ob[roles["seed"]]["period"],
                ob[roles["seed"]]["phase"], 0)]
...
def main(T=2000, T_c=2000, K=60):
    ...
    _, fx_hash = film.fixture_manifest(edges, cfgs, RULEPACK_HASH, roles)
