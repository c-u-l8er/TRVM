...
def ic_films(fx, t0, proj0, T, runner="ref"):
    ...
    st = FX.init_state(fx)      # independent construction (round 7)
    films, costs = [], []
    for e in range(T):
        tc = t0 + e
...
def parity(fx, T, tag, ic32_T=0):
    ...
    # independent-construction check (review round 7): the initial world
    # is derived from the fixture artifact ALONE and must equal the
    # model's boundary film before any transition is compared
    ind = film.film_hash(*FX.state_to_film_args(fx, FX.init_state(fx),
                                                t0 - 1))
    assert ind == film.film_hash(*proj0), \
        f"{tag}: independent init != model boundary"
...
def main():
    ...
    e2a = FX.Fixture({"p3": ("periodic", 3, 0), "seed": ("once", 3)},
                     ["r1", "r2"], ["door"],
                     [("p3", "door"), ("seed", "r1"),
                      ("r1", "r2"), ("r2", "r1")])
    ...
    fanin = FX.Fixture({"p3": ("periodic", 3, 0), "pA": ("periodic", 5, 1)}, [], ["door"],
                       [("p3", "door"), ("pA", "door")])
    ...
    fanout = FX.Fixture({"pB": ("periodic", 4, 2)}, ["rX"], ["dA", "dB"],
                        [("pB", "rX"), ("rX", "dA"), ("rX", "dB")])
    ...
    ring3 = FX.Fixture({"seed": ("once", 3)}, ["rA", "rB", "rC"], ["door"],
                       [("seed", "rA"), ("rA", "rB"), ("rB", "rC"),
                        ("rC", "rA"), ("rA", "door")])
    ...
    muts = [
        ("edge redirected: p3->door becomes p3->r1",
         FX.Fixture({"p3": ("periodic", 3, 0), "seed": ("once", 3)},
                    ["r1", "r2"], ["door"],
                    [("p3", "r1"), ("seed", "r1"),
                     ("r1", "r2"), ("r2", "r1")])),
        ("phase off by one: p3 phase 1",
         FX.Fixture({"p3": ("periodic", 3, 1), "seed": ("once", 3)},
                    ["r1", "r2"], ["door"],
                    [("p3", "door"), ("seed", "r1"),
                     ("r1", "r2"), ("r2", "r1")])),
        ("period changed: p3 period 4",
         FX.Fixture({"p3": ("periodic", 4, 0), "seed": ("once", 3)},
                    ["r1", "r2"], ["door"],
                    [("p3", "door"), ("seed", "r1"),
                     ("r1", "r2"), ("r2", "r1")])),
    ]
